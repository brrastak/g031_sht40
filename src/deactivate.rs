...
use stm32g0xx_hal::{self as hal, gpio::Floating};
use hal::{
    gpio::{
        GpioExt, Input, Output, PushPull, PA2, PA6, PA8
    },
    pac,
    rcc::Rcc,
};
...
impl DeactivatedEpdPins {
    ...
    pub fn steal_epd_pins(rcc: &mut Rcc) -> Self {
        ...
        DeactivatedEpdPins { epd_reset, epd_dc, epd_cs }
    }

    pub fn reactivate(self) -> ActivatedEpdPins {
        let epd_reset = self.epd_reset.into_push_pull_output_in_state(PinState::High);
        ...
        ActivatedEpdPins { epd_reset, epd_dc, epd_cs }
    }
}
